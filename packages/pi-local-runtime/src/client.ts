...
import {
  ...
  LOCAL_RUNTIME_ROUTE,
  LOCAL_RUNTIME_SENTINEL_KEY,
} from "./constants.js";
import type {
  LocalChatRequestMessage,
  LocalChatResponse,
...
export async function syncLocalRuntimeProviderKeys(
  ...
  await syncProviderKey(storage, "openai-codex", runtime.codex.available);
...
export async function resolveApiKey(
  ...
  if (
    shouldUseLocalCodex(runtime, provider) ||
    shouldUseLocalClaude(runtime, provider)
  ) {
  ...
  const nextModel =
    apiKey && proxyEnabled
      ? applyProxyIfNeeded(
          model,
          apiKey,
          (proxyUrl as string | undefined) || undefined,
        )
      : model;
